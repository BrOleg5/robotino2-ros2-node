...
std::array<float, 3> RobotinoKinematics::motor_inverse(float vx, float vy, float omega) {
    std::array<float, 3> wheelVelocity;
    for (unsigned char i = 0; i < 3; i++) {
        wheelVelocity[i] = gearRatio * (-vx * sin(wheelAngle[i]) +
                                        vy * cos(wheelAngle[i]) +
                                        omega * robotRadius) / wheelRadius;
    }
...
std::array<float, 3> RobotinoKinematics::inverse(float vx, float vy, float omega) {
    std::array<float, 3> wheelVelocity;
    for (unsigned char i = 0; i < 3; i++) {
        wheelVelocity[i] = (-vx * sin(wheelAngle[i]) +
                            vy * cos(wheelAngle[i]) +
                            omega * robotRadius) / wheelRadius;
    }
    return wheelVelocity;
...
std::array<float, 3> RobotinoKinematics::direct(const std::array<float, 3>& wheelVelocity) {
    ...
    robotSpeed[2] = wheelRadius * (wheelVelocity[0] + wheelVelocity[1] + wheelVelocity[2]) /
                    (3.f * robotRadius);
    return robotSpeed;
}
